...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public void signIn(@NonNull String username, @NonNull String password, @NonNull Consumer<? super Result<Status, ? extends User>> consumer) {
        ...
        userClient.get(new RequestCallback<>() {
            ...
            public void onRequest(User entity) {
                ...
                var result = Result.of(Status.CORRECT, entity);
            ...
            public void onError(Error e) {
                ...
                if (e.getStatus() == HttpStatus.SC_UNAUTHORIZED) {
                    result = Result.of(Status.INCORRECT, e.getMessage());
                } else {
            ...
            public void onFail(Throwable t) {
                var result = Result.<Status, User>of(Status.FAIL, t);
                consumer.accept(result);
            }
    ...
    @Override
    public void signUp(@NonNull User user, @NonNull Consumer<? super Result<Status, ? extends User>> consumer) {
        if (Authentication.INSTANCE.isAuthenticated())
            throw new IllegalStateException("Authentication instance has already been authenticated");
        userClient.save(user, new RequestCallback<>() {
            @Override
            public void onRequest(User entity) {
                var result = Result.of(Status.VALID, entity);
                consumer.accept(result);
            }
            ...
            public void onError(Error e) {
                ...
                if (e.getStatus() == HttpStatus.SC_BAD_REQUEST) {
                    result = Result.of(Status.INVALID, e.getMessage());
                } else {
                    result = Result.of(Status.ERROR, e.getMessage());
                }
                consumer.accept(result);
    ...
    @Override
    public void profile(@NonNull Consumer<? super Result<Status, ? extends User>> consumer) {
        if (!Authentication.INSTANCE.isAuthenticated())
        ...
        userClient.get(new RequestCallback<>() {
            ...
            @Override
            public void onError(Error e) {
                var result = Result.<Status, User>of(Status.ERROR, e.getMessage());
                consumer.accept(result);
            }
    ...
    @Override
    public void profile(int id, @NonNull Consumer<? super Result<Status, ? extends User>> consumer) {
        if (!Authentication.INSTANCE.isAuthenticated())
            throw new IllegalStateException("Authentication instance has not been authenticated");
